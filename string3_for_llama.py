...
def forward(
    ...
    # STRING
    shift_mask = position_ids >= diag_size
    position_ids_shifted = position_ids.clone()
    position_ids_shifted[shift_mask] = position_ids[shift_mask] - diag_size + local_window

    key_states = apply_rotary_pos_emb(key_states, cos, sin, position_ids_shifted)
    query_states = apply_rotary_pos_emb(query_states, cos, sin, position_ids_shifted)
    ...
    if not has_kv_cache:
        ...
        key_attn = repeat_kv(key_states, self.num_key_value_groups)
        value_attn = repeat_kv(value_states, self.num_key_value_groups)
        
    else:
        key_cache[:, kv_seq_len - 1, :, :] = key_states.transpose(1, 2)
        value_cache[:, kv_seq_len - 1, :, :] = value_states.transpose(1, 2)

    # Shifted-Global Attention - one-pass
    
    if not has_kv_cache:
        q = query_states.transpose(1,2)
        k = key_attn.transpose(1,2)
        ...
        out2, lse2 = do_flash_decoding(query_states, None, None, key_cache, value_cache,
