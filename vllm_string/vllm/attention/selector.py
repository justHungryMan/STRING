...
def get_string_backend(
    ...
    block_size: int,
    is_blocksparse: bool = False,
) -> Type[StringAttentionBackend]:
    if is_blocksparse:
        raise ValueError(
            "String does not support block-sparse attention.")
    backend = which_attn_to_use(num_heads, head_size, num_kv_heads,
                                sliding_window, dtype, kv_cache_dtype,
                                block_size)
    if backend == _Backend.FLASH_ATTN:
        logger.info("Using String Flash Attention backend.")
        from vllm.attention.backends.flash_attn_string import (  # noqa: F401
            StringFlashAttentionBackend)
        return StringFlashAttentionBackend
    else:
        raise ValueError(
            "String requires Flash Attention-2 backend.")
