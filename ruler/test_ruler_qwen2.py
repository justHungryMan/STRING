...
if __name__ == '__main__':
    ...
    print("the maximum input length is ", args.test_max_length)
    ...
    if args.data_dir is None:
        args.data_dir = f"jsonl_data/{args.task}/{open_source_model}-{args.test_max_length}.jsonl"
    ...
    # we use yarn for extrapolation
    training_length = 32768 # hard code from qwen2 report. Qwen2 config file does not contains this value..
    yarn_factor = (test_max_length / training_length) * (1 - args.shifted_ratio)
    
    shifted_offset = int(args.shifted_ratio * test_max_length)
    file_name = os.path.join(pred_save_path, f"string-<S={args.shifted_ratio}L>-<W={args.local_value}>-<F={yarn_factor}>.jsonl")
    tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=True, trust_remote_code=True)

    if shifted_offset > 1:
        print(f"========== Test Length: {test_max_length} & Shifted offset: {shifted_offset} & Training Length: {training_length} ==========")
        from string_for_qwen2 import replace_with_string
        replace_with_string(test_max_length, shifted_offset=shifted_offset, small_local_value=args.local_value, yarn_factor=yarn_factor)
    else:
